...
movies = db.query(Movie).limit(10).all() #all pour afficher toutes les lignes récup
for movie in movies:
...
# %%
highest_rated_movies = ( db.query(Movie.title, Rating.rating)
.join(Rating)
...
# En spécifiant une colonne de jointure
highest_rated_movies = ( db.query(Movie.title, Rating.rating)
.join(Rating)
